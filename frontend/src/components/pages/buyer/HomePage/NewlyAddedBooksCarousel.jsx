...
const NewlyAddedBooksCarousel = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const [currentSlide, setCurrentSlide] = useState(0);

  const {
    newlyAddedBooks: { books = [], totalBooks = 0 } = {}, // Destructure nested safely
    loadingNewlyAddedBooks: isLoading,
    errorNewlyAddedBooks: error,
  } = useSelector((state) => state.books);

  useEffect(() => {
    dispatch(getNewlyAddedBooksAction({ limit: 10, page: 1 }));
  }, [dispatch]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Link
          onClick={() => navigate("/newlyAddedBooksPage")}
          sx={{
            color: "#008000",
            fontSize: "14px",
            fontWeight: "bold",
...
export default NewlyAddedBooksCarousel;
